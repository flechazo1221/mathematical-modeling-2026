...
# The candidate lives one level below the package directory; keep its output
# local while resolving the repository root independently.
...
def run_case(module, cfg, latent: dict) -> dict:
    ...
    def moisture_predictor(grid, dt, cenv, x0):
        ...
        # Promotion route is deliberately single-field: C_b=0 and no internal
        # gas inventory.  A cumulative surface export ledger is bookkeeping,
        # not a phase-resolved state variable.
    ...
    def add_latent_source(grid, dt, cenv):
        ...
        context["source_trace"][time_key] = {
            ...
            "route_contract": "C_b=0; C_l=C; no internal gas inventory",
            "bound_fraction": 0.0,
            "release_tau_s": 0.0,
...
SCENARIOS = {
    "no_latent": {"enabled": False, "proxy_scale": 0.0, "L_v_J_per_kg": 2.4e6,
                  "route_contract": "C_b=0; C_l=C; no internal gas inventory", "label": "无潜热可迁移水基线"},
    "low_proxy": {"enabled": True, "proxy_scale": 1.0, "L_v_J_per_kg": 1.2e6,
                   "route_contract": "C_b=0; C_l=C; no internal gas inventory", "label": "低档表面等效潜热"},
    "nominal_proxy": {"enabled": True, "proxy_scale": 1.0, "L_v_J_per_kg": 2.4e6,
                       "route_contract": "C_b=0; C_l=C; no internal gas inventory", "label": "名义表面等效潜热"},
    "high_proxy": {"enabled": True, "proxy_scale": 1.0, "L_v_J_per_kg": 4.8e6,
                    "route_contract": "C_b=0; C_l=C; no internal gas inventory", "label": "高档表面等效潜热"},
}
...
def component_split_rows(rows: list[dict]) -> list[dict]:
    """Keep the old optional output schema empty: C_b=0 has no toy branch."""
    return []


def component_validations() -> list[dict]:
    return [
        {"id": "ROUTE-V01-single-field", "status": "PASS", "evidence": "C_b=0 and C_l=C; no bound-water equation or internal gas inventory is present in the active route."},
        {"id": "ROUTE-V02-surface-latent", "status": "INCONCLUSIVE", "evidence": "The same total-moisture Robin j_v is reused for q_lat=Lv*j_v; full-run audit is still required."},
        {"id": "ROUTE-V03-physical-validation", "status": "BLOCKED", "evidence": "No phase-resolved observations are available; the candidate is an equivalent surface boundary, not a measured phase model."},
    ]
...
def main() -> None:
    ...
    write_csv(OUT / "parameter-register.csv", [
        {"parameter": "j_v", "definition": "rho_d*(C_surface_pred-C_inf)/R_m", "unit": "kg/m2/s", "source": "existing total-moisture Robin boundary flux; q_w=rho_d*C", "status": "EQUIVALENT_SURFACE_FLUX"},
        ...
        {"parameter": "L_v", "definition": "multiplier in q_lat=Lv*j_v surface heat flux", "unit": "J/kg", "source": "2.4e6 nominal scale; no project-specific enthalpy/DSC source", "status": "NOT_IDENTIFIED"},
        ...
        {"parameter": "phase-resolved closures", "definition": "not part of C_b=0 candidate", "unit": "various", "source": "not present in current A attachments; physical identification remains blocked", "status": "MISSING"},
    ])
    ...
    (OUT / "H2-选择包.md").write_text(
        "# H2-选择包：潜热与最小三组分工程候选\n\n"
        "状态：**不升级为正式主模型；H2 保留人工决策。** 本候选只包含 `C_b=0` 的单一可迁移水场和表面等效潜热边界。\n\n"
        "本轮以温度场无潜热候选为配对基线，复用现有总水分 Robin 表面通量 `j_v`，并用同一个 `q_lat=Lv*j_v` 进入能量边界。题目附录已给 `rho/cp/k/D` 经验关系，附件1/2已给边界历程和半径历程；`L_v` 仍只能作敏感性，物理验证和相分辨识别保持阻塞。\n\n"
        "## 已进行的优化与后续优化方向\n\n"
        "|项目|状态|成本|风险|保留/回退|\n|---|---|---|---|---|\n"
        "|无潜热 vs 边界 `q_lat=-Lv*j_w` 的 Q2/Q3/Q4 配对|已运行|中|总边界通量代理，不是两相模型|保留为论文扩展/敏感性，不升级|\n"
        "|低/标准/高 Lv 潜热敏感性|已运行|中|Lv 无本题独立焓源|保留为风险范围|\n"
        "|同乘积 scale/Lv 交换|不作为本路线结论|低|Lv 仍未独立识别|不写成定参证据|\n"
        "|结合水主方程|明确不加入|低|本路线固定 C_b=0|不得写入候选或正式实现|\n"
        "|内部气态水库存|明确不加入|低|仅可记录表面累计输出|不得称内部相分辨库存|\n"
        "|独立空间/时间梯子|已运行|高|Q3/Q4 事件时间仍未达到 60 s|保留原始差异，不宣称收敛|\n\n"
        "## 三档建议\n\n"
        "1. **候选范围**：单一 `C_l=C` + 表面 `q_lat=Lv*j_v`，附 `L_v=1.2/2.4/4.8 MJ/kg` 敏感性、热量账和局限。\n"
        "2. **明确不加入**：`C_b` 方程、内部汽相库存、固定液/汽比例和第二套相通量。\n"
        "3. **仍不可宣称**：独立识别 `L_v`、相分辨 `Gamma/j_v` 或物理验证通过。\n\n"
    ...
    # Replace inherited report text with the promotion-route contract.  The
    # solver core is reused, but bound-water and internal-gas narratives are
    # intentionally excluded from this candidate's published outputs.
    (OUT / "候选方案对比.md").write_text(
        "# H2 重开候选：内部温度场 + 表面等效潜热\n\n"
        "状态：PROTOTYPE/BLOCKED。路线为 `C_b=0`、`C_l=C`、无内部气态水库存；同一个总水分 Robin 表面通量 `j_v` 进入 `q_lat=Lv*j_v`。\n\n"
        "`L_v=2.4e6 J/kg` 为名义值，`1.2e6/4.8e6 J/kg` 为敏感性。Q4 继续使用 `q_w=rho_d*C` 和移动域几何账。完整 Q4 运行、60 s 收敛和物理验证必须分别登记。\n",
        encoding="utf-8",
    )
    (OUT / "H2-选择包.md").write_text(
        "# H2 重开候选\n\n"
        "本候选只包含 `C_b=0` 的单一可迁移水场和表面等效潜热边界，不包含结合水主方程、内部气态水库存或固定液/汽比例。\n\n"
        "同一个 `j_v` 同时用于表面输出记账和 `q_lat=Lv*j_v`；`L_v` 三档仅是敏感性。数值运行通过不等于物理验证通过，严格 Q4 60 s 门槛和相分辨识别限制必须保留。\n",
        encoding="utf-8",
    )
    ...
    (OUT / "中文结论.md").write_text(
        ...
        "`Lv=2.4e6 J/kg` 仅是名义工程尺度，低/高档只作敏感性；本路线不加入结合水方程或内部气态水库存。由于相分辨识别不可用，handoff 保持 `PROTOTYPE`/H2 阻塞状态，绝不标记 `TEAM_APPROVED`。\n\n"
    ...
    handoff = {"schema_version": "1.0", "stage": "PROTOTYPE", "status": "BLOCKED",
               ...
               "assumptions": ["no-latent internal-temperature candidate is the paired numerical baseline", "j_v reuses the total moisture Robin boundary flux as the equivalent surface flux", "all upstream and formal directories are read-only"],
               "unknowns": manifest["unknowns"], "claims": manifest["claims"], "warnings": manifest["warnings"],
               "required_next_actions": ["do not promote; retain physical-identification limits", "audit the single-field route and do not add phase inventories", "repeat independent Q4 space/time gates"],

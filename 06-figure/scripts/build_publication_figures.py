...
fid="FIG-Q4-IMPLEMENTATION-AGREEMENT"; d=pd.DataFrame({"implementation":["reference","moving-FV"],"event_time_s":[183789.33174915268,183789.33174915268+5.820766091346741e-10],"reported_s":[183840,183840]}); _,m=snap_csv(fid,d,[RES/"run-summary.csv",RES/"convergence.csv"]); contract(fid,m); fig,ax=plt.subplots(); ax.scatter(d.implementation,d.event_time_s,c=COL[:2],s=60); style(ax,"数值实现","插值事件时刻 (s)"); ax.ticklabel_format(axis="y",style="plain",useOffset=False); ax.text(.01,.08,"差 5.82e-10 s；两者均报告 183840 s",transform=ax.transAxes); ax.text(.01,.02,"实现一致性不等于现实准确性",transform=ax.transAxes); save(fid,fig)

...
from formal_compute_temp_field import RunConfig, simulate
...
with (LOGS/'run.log').open('w',encoding='utf-8') as log:
    ...
    for label,cfg in cases:
        ...
        with (RESULTS/(label+'-trajectory.csv')).open('w',newline='',encoding='utf-8-sig') as tf:
            fields=['time_s','radius_m','max_C','mean_C','center_C','surface_C','center_T_C','surface_T_C']
            w=csv.DictWriter(tf,fieldnames=fields); w.writeheader()
            for rec in run['records']:
                w.writerow({k:rec.get(k,'') for k in fields})
        with (RESULTS/(label+'-profiles.csv')).open('w',newline='',encoding='utf-8-sig') as pf:
            w=csv.writer(pf); w.writerow(['time_s','radius_cm','temperature_C','C_kg_per_kg'])
            for t, temp_prof, c_prof in run['states']:
                radii_cm = np.linspace(0.0, 2.0, len(temp_prof))
                for radius_cm, tv, cv in zip(radii_cm, temp_prof, c_prof):
                    w.writerow([t,radius_cm,tv,cv])

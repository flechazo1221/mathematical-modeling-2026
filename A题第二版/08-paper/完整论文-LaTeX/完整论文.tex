...
\subsection{模型假设}
\begin{enumerate}[label=\arabic*.,leftmargin=2em,itemsep=0.35em,topsep=0.3em]
\item 将药材视为连续、均质的轴对称圆柱体，忽略周向差异；初始温度场和干基含水率场均匀。
\item 轴线和中截面采用对称边界；侧面和端面与烘房之间的传热、传质均采用 Robin 交换边界，传递系数按题设取值。
\item 物性和扩散系数直接采用题设经验关系，并在状态相关模型中随含水率、温度更新，不额外引入无法由现有数据识别的参数。
\item 所有计算结果均不计汽化潜热、吸附热和反应热。因此，文中“实际温度”特指由显热方程预测的药材内部温度，不等同于实测温度，也不代表完整的热质相变耦合结果。
\item $0$--$4$ h 内的温度和水分环境观测按数据预处理方案插值；$4$ h 后采用末一小时环境数据的平均值作恒定延拓。该延拓仅是长时间数值积分所需的计算假设，不视为 $4$ h 后的实测数据或未来预测数据。
\item Q4 采用径向仿射收缩，轴向长度保持不变；半径由附件 2 的观测序列经保形插值得到。收缩过程中干固体总质量守恒，随动控制体的几何和干固体密度同步更新。
\end{enumerate}

...
3. 三类图体系：按 `raw_q1_*`、`process_q1_*`、`result_q1_*` 等命名；仅保留回答问题、验证模型或支撑论文主张的图，不设置固定数量或图型配额。每张正式图同时导出 SVG 与 300 DPI PNG。
...
代码、结果表、全部保留图和复现清单完成作者自检并冻结后，按 `../../../../references/Subagent调度.md` 派发独立质检 Subagent。质检者重跑唯一复现命令，核对哈希、环境、种子、参数、关键数值、约束、边界、量纲、统计计算和各子问题图表语义；带全部 `--questions` 的 `figure_audit.py --strict` 结果和实际 PNG/灰度预览作为证据输入。`P2` 未返回 `PASS` 不得进入论文阶段。

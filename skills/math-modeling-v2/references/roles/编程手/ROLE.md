...
本文件不是可发现的 Skill 入口。新分阶段工作流使用 `skills/math-modeling-prototype/`、`skills/math-modeling-compute/` 与 `skills/math-modeling-figure/`；仅在迁移旧项目或查阅旧版实现约定时读取本指南。
...
- `figures/` 中回答子问题、验证模型或支撑论文主张所需的图。每张图必须绑定具体问题或证据用途；不设置固定图数、类别数或图型配额。文件名使用 `raw_q1_*`、`process_q1_*`、`result_q1_*` 等可追溯格式；同一图的 SVG、PNG 和灰度预览视为一个图表资产。
- `results/复现清单.json`。
...
6. 将 `scripts/plot_style.py` 或 MATLAB 的三个出版绘图工具复制到 `PROJECT_ROOT/utils/` 后使用。按 `../../../tools/figure/SKILL.md` 的视觉论证流程，仅生成回答问题、验证模型或支撑论文主张所需的图；每张保留图都必须有证据用途。不把不同重要性的面板机械等分，不用长标题、密集逐点标记、装饰性纹理或面板内重复图例堆成仪表盘；统计标注必须由代码计算，官方模板要求优先于内置基线。
...
- `P2`：代码、结果、全部保留图和复现清单冻结后，质检 Subagent 独立运行唯一复现命令并核对输入哈希、种子、关键数值、边界、量纲、各子问题图表语义及文件完整性。机械图审继续使用带全部 `--questions` 的 `figure_audit.py --strict`，Subagent 负责实际读图，检查单图核心结论、主次面板、图例、统计区间和最终尺寸可读性，而不是只确认文件能打开。

...
def audit_figure_directory(
    ...
    *,
    ...
    if min_per_category < 0:
        raise ValueError("每类候选图最低数量不能小于 0")
    ...
    if require_categories:
        for prefix in REQUIRED_CATEGORIES:
            ...
            if min_per_category and count < min_per_category:
                issues.append({
                    "severity": "FAIL",
                    "message": (
        ...
        for question in normalized_questions:
            markers = tuple(f"{prefix}{question}_" for prefix in REQUIRED_CATEGORIES)
            if not any(stem.startswith(markers) for stem in by_stem):
                issues.append({
                    "severity": "FAIL",
                    "message": f"子问题 {question} 缺少可追溯候选图",
                })
...
def main() -> int:
    parser = argparse.ArgumentParser(description="检查论证图的子问题追溯、SVG 可编辑文本和 PNG DPI")
    parser.add_argument("figures_dir", help="PROJECT_ROOT 下的 figures 目录")
    parser.add_argument("--min-dpi", type=int, default=300)
    parser.add_argument("--min-per-category", type=int, default=0, help="可选的每类逻辑候选图最低数量；默认不设数量配额")
    parser.add_argument(
        "--questions",
        nargs="+",
        default=(),
        metavar="Q",
        help="预计由图表支撑的子问题标识，例如 q1 q2 q3",
    )

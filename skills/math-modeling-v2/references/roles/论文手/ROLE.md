...
本文件不是可发现的 Skill 入口。新分阶段工作流使用 `skills/math-modeling-paper/`；仅在迁移旧项目或查阅旧版排版约定时读取本指南。

## 开始门禁

开始写作前先在进度更新中报告输入检查结果。题目、两个建模交付物、可运行代码、实际结果表格、支撑核心主张所需且覆盖全部子问题的已核验图表或文献任一缺失时，回退补齐，不要直接写论文。图表以证据需要为准，不设置固定数量或类型配额。

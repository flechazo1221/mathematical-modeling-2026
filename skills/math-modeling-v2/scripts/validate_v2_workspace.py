...
REQUIRED_GATES = {
    ...
    "APPENDIX": ("H3-claims.json", {"TEAM_APPROVED"}),
...
STAGE_PREREQUISITES = {
    "DESIGN": ("LITERATURE",),
    "APPENDIX": ("COMPUTE", "FIGURE"),
    "PAPER": ("DESIGN", "COMPUTE", "EVIDENCE", "FIGURE", "APPENDIX"),
    "AUDIT": ("PAPER",),
}
...
def validate_workspace(project_root: Path, require_complete: bool = False) -> list[str]:
    ...
    config = load_json(root / ".workflow" / "config.json", errors)
    ...
    for stage in completed:
        ...
        if stage == "DESIGN" and config is not None and config.get("literature_review_gate", False):
            validate_decision(
                root / "decisions" / "L1-literature.json",
                {"TEAM_APPROVED"},
                errors,
            )

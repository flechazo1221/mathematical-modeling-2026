...
AFTER_STAGE = {
    ...
    "FIGURE": ("APPENDIX", "RUNNING"),
    "APPENDIX": ("PAPER", "RUNNING"),
    "PAPER": ("AUDIT", "RUNNING"),
    "AUDIT": ("H4", "WAITING_FOR_TEAM"),
}
AFTER_GATE = {"H0": "INTAKE", "H1": "LITERATURE", "L1": "DESIGN", "H2": "COMPUTE", "H3": "FIGURE"}
GATE_FILES = {
    "H0": "H0-selection.json",
    "H1": "H1-problem.json",
    "L1": "L1-literature.json",
    "H2": "H2-model.json",
    "H3": "H3-claims.json",
...
def start_stage(root: Path, stage: str, thread_id: str, host_id: str | None) -> None:
    ...
    if stage == "DESIGN":
        ...
        if config.get("literature_review_gate", False):
            decision = load_json(root.resolve() / "decisions" / GATE_FILES["L1"])
            if decision.get("status") != "TEAM_APPROVED":
                raise ValueError("DESIGN requires TEAM_APPROVED L1 literature review")
            if not decision.get("confirmed_by") or not decision.get("confirmed_at"):
                raise ValueError("L1 literature approval lacks named reviewers or timestamp")
    if stage == "LITERATURE":
...
def finish_stage(root: Path, stage: str) -> None:
    ...
    config = load_json(project_files(root)[0])
    if stage == "LITERATURE" and config.get("literature_review_gate", False):
        next_stage, status = "L1", "WAITING_FOR_TEAM"
    else:

...
- `02-literature/L1-文献审核包.md`
- `02-literature/handoff.json`

`L1-文献审核包.md` must let the team audit the literature work before DESIGN. Include the searched and actually supplied source scope, usable/unusable counts, source-by-source relevance, key supported facts, contradictions, unresolved gaps, transfer risks, proposed sources permitted as DESIGN inputs, sources proposed for exclusion, and a neutral approval question. Clearly distinguish downloaded full text, user-supplied full text, abstract-only metadata, and unavailable records. Do not ask the team to approve a model route at L1.
...
Stop after the reading handoff and wait for the team to review `L1-文献审核包.md`. Do not design candidates, choose a model, implement code, modify the user-supplied literature, or imply L1 approval. DESIGN may consume the literature synthesis only after `decisions/L1-literature.json` is explicitly `TEAM_APPROVED`.

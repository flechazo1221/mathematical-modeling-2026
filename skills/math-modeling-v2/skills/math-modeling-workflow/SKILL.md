...
SELECTION → H0 → INTAKE → H1 → WAITING_FOR_LITERATURE → LITERATURE
       → L1 → DESIGN → PROTOTYPE → H2 → COMPUTE → EVIDENCE → H3
       → FIGURE → optional C1 → APPENDIX → PAPER → AUDIT → H4 → COMPLETE
```
...
Supervision is mandatory for `SELECTION`, `INTAKE`, `DESIGN`, `PROTOTYPE`, `COMPUTE`, `EVIDENCE`, `FIGURE`, `APPENDIX`, `PAPER`, and `AUDIT`. It starts before task dispatch and ends only at a validated handoff, an immediate deviation stop, or a user-requested termination.
...
After H1 is approved, advance the gate and stop at `WAITING_FOR_LITERATURE`. Do not create the LITERATURE reading task in the same turn as H1 approval.

At this checkpoint, use `$math-modeling-literature` to prepare the literature-acquisition handoff rather than requiring the user to discover every paper manually. Derive a small, H1-traceable set of Chinese and English queries and write the search plan under `PROJECT_ROOT/literature/acquisition/`. When CNKI is relevant, the handoff must include the official CNKI professional-search URL and copy-ready professional-search expressions. Present that link and the expressions to the user, then end the turn with an explicit request that they open the page, complete any login or CAPTCHA themselves, and reply that the page is ready.

When the user returns and explicitly says the CNKI page is open or ready, use the available browser-control capability to connect to that existing tab and continue the CNKI-assisted acquisition protocol in `$math-modeling-literature`. Do not create a new task merely to control the browser. Search, screen, record metadata, and download only copies the user's current authorized session can lawfully access. Never solve or bypass a CAPTCHA, automate credentials, or circumvent a paywall. If either blocks progress, pause at `WAITING_FOR_LITERATURE`, tell the user the exact manual step required, and resume only after they return.

After acquisition, stop again so the user can review the selected/downloaded set. Create the LITERATURE reading task only after the user has returned and at least one supported literature file exists under `literature/input/`. User-supplied files remain an equally valid path; the CNKI-assisted path is not mandatory when the user already has suitable literature or CNKI is irrelevant.

For `LITERATURE`, create a fresh task using only `$math-modeling-literature-reading`. Give it the approved H1 decision, original problem context, `literature/input/` as read-only user material, and `02-literature/` as its sole write directory. Require `文献阅读结果.md`, `文献清单.json`, `L1-文献审核包.md`, and a `PASS` handoff. After validating that handoff and all declared hashes, enter the L1 human gate and present the review package to the user. Do not start DESIGN until `decisions/L1-literature.json` is explicitly `TEAM_APPROVED` by named team members.

For `APPENDIX` (the `07-appendix/` stage), create a fresh task using only `$math-modeling-appendix` after FIGURE passes and optional C1 is resolved. Give it the verified contest/year rules, `.workflow/ai-usage-log.json`, source-code directories, approved figures, and any interaction-platform screenshots. Require both Markdown deliverables and a `PASS` handoff. It must not invent actual AI-use history; unresolved mandatory evidence must remain visibly marked for team review or cause `BLOCKED` when compliance cannot be established. This is the mandatory stage immediately before final paper generation.

For `PAPER` (the `08-paper/` delivery stage), do not create the task until DESIGN, COMPUTE, EVIDENCE, FIGURE, and APPENDIX have validated `PASS` handoffs and H3 has approved the claims. Include the frozen APPENDIX Markdown outputs as read-only inputs and require the paper task to integrate the declaration and appendix without silently altering their compliance content. The new task prompt must name the target contest and year, required output formats, approved paper type or types when known, and require `math-modeling-paper` to apply both its hard writing gate and the bundled `references/paper-template-2026/INTEGRATION.md` routing contract. Explicitly instruct it to organize the paper by each subproblem's approved route, use only executed and confirmed results, auto-bind upstream tables/figures/metrics, explain rather than list results, preserve quantitative validation details, analyze limitations/sensitivity/scope, run formula-code-result-figure-text consistency checks, and polish only after those checks pass. Do not pass the adjacent source repository as a runtime dependency.
...
At H0, H1, L1, H2, H3, and H4 set `status` to `WAITING_FOR_TEAM`, report the choice package, and end the turn. H0 selects the problem after the fixed weighted comparison; the controller and selection task must not choose on the team's behalf. L1 approves the searched/downloaded source set, literature synthesis, evidence gaps, transfer limits, and sources permitted as DESIGN inputs. Do not create the next task until the corresponding decision file satisfies `shared/schemas/decision.schema.json` and contains a real team approval. H1 approval leads to a separate `WAITING_FOR_LITERATURE` checkpoint; this is an input pause, not another approval gate.
...
- Appendix code, AI disclosure, references, or screenshots: rerun APPENDIX.

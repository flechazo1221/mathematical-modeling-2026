...
3. **文字遮盖 / 重叠**
   ...
   - 多位数字是否逐字符完整显示，尤其是窄柱内部、坐标轴上缘和图例下方？标签包围盒放不下时必须外置并预留空间。
   - x 轴刻度标签有没有挤成一团、互相穿插？
...
9. **背景与透明图层**
   ...
   - 若背景分区不编码连续数值，优先用细边界线、括号或标签；确需填充时必须拆成互不重叠的区间。
...
| 数字在窄柱/窄图元内显示不全 | 将数字移到柱顶或柱侧；设置明确的轴边距；重渲后逐字符核对 |
| 半透明背景叠色不一致 | 删除非必要填充；改用 `axvline`/括号；或拆成互不重叠的 patch |

...
def apply_compact_paper_style(lang: str = "en", scale: float = 1.0) -> dict:
    """Apply a light, compact paper style without forcing a figure size.

    Font selection is delegated to setup_style so Chinese glyph coverage is
    preserved.  The caller must still size the canvas from label density.
    """
    from setup_style import setup_style

    info = setup_style(
        journal="general", lang=lang, use_sciplots=False,
        constrained_layout=False,
    )
    plt.rcParams.update({
        "font.size": 8.0 * scale,
        "axes.titlesize": 9.0 * scale,
        "axes.labelsize": 8.4 * scale,
        "xtick.labelsize": 7.6 * scale,
        "ytick.labelsize": 7.6 * scale,
        "legend.fontsize": 7.2 * scale,
        "axes.linewidth": 0.8,
        "grid.color": "#D9D9D9",
        "grid.linewidth": 0.55,
        "grid.linestyle": "--",
...
def style_compact_axis(ax, grid_axis: str = "y", grid: bool = True) -> None:
    """Use light axes and redundant-friendly visual scaffolding."""
...
def add_shared_legend_safe(
    fig, axes, *, ncol: int, y: float = 0.995, order=None,
    columnspacing: float = 0.7, handletextpad: float = 0.25,
):
    """Add one de-duplicated figure legend in a dedicated top band.

    This does not silently move a suptitle.  Put the suptitle below the legend
    (typically y<=0.91) and reserve axes top<=0.74, then run audit_layout().
    """
    by_label = {}
    for ax in list(axes):
    ...
    labels = list(order) if order is not None else list(by_label)
    labels = [label for label in labels if label in by_label]
    handles = [by_label[label] for label in labels]
    return fig.legend(
        handles, labels, loc="upper center", ncol=ncol, frameon=False,
        bbox_to_anchor=(0.5, y), columnspacing=columnspacing,
        handletextpad=handletextpad,
    )


def set_grouped_category_ticks(
    ax, tick_labels, groups, *, rotation: float = 45,
    group_y: float = -0.27, separators: bool = True,
):
    """Lay out dense categorical ticks with non-repeated group labels.

    groups is an iterable of ``(label, first_index, last_index)``.  Every
    category remains present; only repeated text is factored into a group band.
    """
    positions = list(range(len(tick_labels)))
    ax.set_xticks(positions, tick_labels, rotation=rotation,
                  ha="right" if rotation else "center")
    groups = list(groups)
    for label, first, last in groups:
        if not (0 <= first <= last < len(positions)):
            raise ValueError(f"Invalid group range {(first, last)}")
        ax.text((first + last) / 2, group_y, label,
                transform=ax.get_xaxis_transform(), ha="center", va="top")
    if separators:
        for (_, _, last), (_, first, _) in zip(groups, groups[1:]):
            ax.axvline((last + first) / 2, color="#E6E6E6", linewidth=0.7)


def finalize_compact_figure(fig, *, top: float = 0.74, bottom: float = 0.18,
                            left: float = 0.12, right: float = 0.99,
                            wspace: float = 0.20, hspace: float = 0.28) -> str:
    """Reserve explicit bands for shared legend, title, axes, and group labels."""
    try:

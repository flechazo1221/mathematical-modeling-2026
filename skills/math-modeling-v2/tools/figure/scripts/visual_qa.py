...
- ``audit_layout(fig)`` —— 返回 ``[(severity, msg), ...]``：
  ...
  * **结构文字碰撞**（WARN）：总标题、共享图例、轴标题、分组标签或面板标签相交。
  * **表格文字溢出**（WARN）：单元格文字超出本单元格边界。
  * **透明背景叠色**（WARN）：半透明 patch 互相覆盖造成非预期深浅色块。
...
def audit_layout(fig, clip_tol_px: float = 2.0, overlap_tol_px: float = 1.0
    ...
    if suptitle is not None and suptitle.get_visible() and suptitle.get_text().strip():
        try:
            sb = suptitle.get_window_extent(renderer)
            for legend in fig.legends:
                if legend.get_visible() and _bbox_overlap(sb, legend.get_window_extent(renderer), 1.0):
                    collision_names.append("总标题—共享图例")
        except Exception:
            pass

    for index, ax in enumerate(fig.axes, start=1):
        structural = []
        for name, artist in (("标题", ax.title), ("x轴标题", ax.xaxis.label),
                             ("y轴标题", ax.yaxis.label)):
            try:
                if artist.get_visible() and artist.get_text().strip():
                    structural.append((name, artist.get_window_extent(renderer)))
            except Exception:
                pass
        for i, artist in enumerate(ax.texts, start=1):
            try:
                if artist.get_visible() and artist.get_text().strip():
                    structural.append((f"结构文字{i}", artist.get_window_extent(renderer)))
            except Exception:
                pass
        legend = ax.get_legend()
        if legend is not None and legend.get_visible():
            try:
                structural.append(("图例", legend.get_window_extent(renderer)))
            except Exception:
                pass
        for i, (name_a, box_a) in enumerate(structural):
            for name_b, box_b in structural[i + 1:]:
                if _bbox_overlap(box_a, box_b, 1.0):
                    collision_names.append(f"子图{index}:{name_a}—{name_b}")
    if collision_names:
        uniq = list(dict.fromkeys(collision_names))[:8]
        issues.append((
            "WARN",
            f"检测到结构文字碰撞：{uniq}。为图例、总标题和分组标签分配独立带；"
            "优先增加画布/边距或缩短标签，不要只缩小字号。"
        ))

    # ---- 5. 表格单元格文字溢出 ----
    overflow = []
    for ax_index, ax in enumerate(fig.axes, start=1):
        for table in ax.tables:
            for key, cell in table.get_celld().items():
                try:
                    cb = cell.get_window_extent(renderer)
                    tb = cell.get_text().get_window_extent(renderer)
                    if (tb.x0 < cb.x0 - 1.0 or tb.x1 > cb.x1 + 1.0
                            or tb.y0 < cb.y0 - 1.0 or tb.y1 > cb.y1 + 1.0):
                        overflow.append(f"子图{ax_index}单元格{key}")
                except Exception:
                    continue
    if overflow:
        issues.append((
            "WARN",
            f"表格文字可能溢出：{list(dict.fromkeys(overflow))[:8]}。"
            "请增加行高/列宽或按语义换行；数学表达用 mathtext，最小字号保持>=6 pt。"
        ))
    ...
    for ax in fig.axes:
        ...
        for i, first in enumerate(boxes):
            for second in boxes[i + 1:]:
                if _bbox_overlap(first, second, 2.0):
...
def _bbox_overlap(a, b, tol: float = 0.0) -> bool:
    """Whether two display-coordinate boxes overlap beyond tolerance."""

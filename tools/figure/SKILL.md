...
- **信息不重复**：每个面板回答唯一问题；图型服从数据与论证需要，不用不同形式重复展示相同数据（详见 `design_theory.md` §11 反冗余清单）
...
数学建模场景（两种语言均适用）：按论证需要从原始数据、模型过程、最终结果和稳健性证据中选图。每个核心主张使用最小充分图表，不机械规定类别、数量或图型种类；重复、装饰性、无法支持正文结论或隐藏失败案例的图应删除。
...
- `figure_audit.py`：检查图表能否追溯到预计由图表支撑的子问题，不设置默认数量配额。
...
6. **最小充分**——图型、面板和数量由主张—证据链决定；每个面板回答唯一问题，不用不同形式重复相同信息。

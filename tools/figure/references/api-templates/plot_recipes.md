...
| 类别 | 目的 | 推荐图表类型 | 产出阶段 | 选择原则 |
|------|------|-------------|---------|-----------|
| **① 原始数据可视化** | 理解数据特征，发现潜在规律 | 直方图、箱线图、密度图、热力图、散点图矩阵、折线图、堆叠柱状图 | 数据审计 | 按实际分布、关系或异常问题选择 |
| **② 中间产物可视化** | 展示求解过程，验证模型行为 | 收敛曲线、分组柱状图、灵敏度折线、残差图、QQ图、混淆矩阵热力图、学习曲线 | 模型验证 | 按收敛、误差、敏感性或对比问题选择 |
| **③ 最终结果可视化** | 支撑核心结论，论文直接使用 | 对比柱状图、预测折线+CI、雷达图、气泡图、组合图、地图热力图 | 结果表达 | 按批准主张和证据形态选择 |

**全局要求**：图型服从数据语义和论证问题；不得为追求表面多样性而重复展示同一证据。

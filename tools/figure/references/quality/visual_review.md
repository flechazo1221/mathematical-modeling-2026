...
3. **文字遮盖 / 重叠**
   ...
   - 多位数字是否完整显示，尤其是窄柱内部、坐标轴上缘和图例下方？不得只确认“存在标签”，必须能逐字符读出。
   - 标签若放在图元内部，其渲染包围盒是否真正放得下？放不下就移到图元外侧，并为标签预留轴内空白。
   - x 轴刻度标签有没有挤成一团、互相穿插？
...
9. **背景与透明图层**
   - 各子图底色是否一致？同一坐标区是否因多个 `axvspan`、阴影带或透明补丁叠加而出现意外深浅色块？
   - 若背景分区不编码连续数值，优先用细边界线、括号或标签，不用交叠的半透明色块。
   - 确需多个填充区时，先拆成互不重叠的区间；不得依赖 alpha 混色产生目标颜色。
...
| 数字在窄柱/窄图元内显示不全 | 将数字移到柱顶或柱侧；设置明确的 `ylim`/边距；渲染后逐字符核对，不用裁切文本掩盖问题 |
| 半透明背景叠色不一致 | 删除非必要填充；改用 `axvline`/括号/顶部短线；或把区间拆为互不重叠的 patch |
